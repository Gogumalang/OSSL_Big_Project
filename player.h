...
void update_player(node *h); // 선수 정보 수정 함수 // 
void delete_player(node *h); // 선수 정보 삭제 함수 //
...
void buy_player(node *h,int *account,int n); // 선수 구입 함수 // 
void sell_player(node *h,int *account,int n); // 선수 판매 함수 //
void cmp_player(node *h); // 선수 비교 함수 //


int server_login(); // 서버 로그인 함수 //
int select_server(); // 서버 메뉴 함수 //
int select_client(); // 유저 메뉴 함수 //
void deallocation(node *h); // 링크드 리스트 할당을 해제 함수 //

void read_all_players(node *h); // 모든 선수 정보를 보는 함수 //

int sign_in_or_up(); // 회원가입 로그인 선택하기 //
void sign_in(); // 유저로서 회원가입 하기 //
int sign_up(); // 유저로서 로그인 하기 //
